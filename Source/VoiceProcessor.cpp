...
std::vector<juce::MidiMessage> VoiceProcessor::processSample(const std::optional<std::vector<juce::MidiMessage>>& enteredMessages, int channel, bool bypassed) {
    ...
    while (!unprocessedBuffer.empty() && unprocessedBuffer.front().time <= getReadPosition()) {
        ...
        if (message.isController()) {
            readPositionCCStates[message.getControllerNumber()] = message.getControllerValue();
        }
        if (message.isNoteOn()) {
            readPositionHeldNotes[message.getNoteNumber()] = message.getVelocity();
        }
        if (message.isNoteOff()) {
            readPositionHeldNotes[message.getNoteNumber()] = 0;
        }
        if (message.isProgramChange()) {
            readPositionProgram = message.getProgramChangeNumber();
        }

        if (!configuration->isBlocked(message)) {
    ...
    if (enteredMessages.has_value()) {
        for (const auto& message : enteredMessages.value()) {
            if (bypassed) {
                ...
                unprocessedBuffer.emplace_back(message, getWritePosition());
                if (message.isController()) {
                    writePositionCCStates[message.getControllerNumber()] = message.getControllerValue();
                }
                if (message.isNoteOn()) {
                    writePositionHeldNotes[message.getNoteNumber()] = message.getVelocity();
                }
                if (message.isNoteOff()) {
                    writePositionHeldNotes[message.getNoteNumber()] = -1;
                }
                if (message.isProgramChange()) {
                    writePositionProgram = message.getProgramChangeNumber();
                }
